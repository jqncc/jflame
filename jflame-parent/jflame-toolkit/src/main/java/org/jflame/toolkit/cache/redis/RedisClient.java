...
public interface RedisClient {
    ...
    default byte[] rawKey(String key) {
        assertNotNull(key, "cache key not be null");
        return getKeySerializer().serialize(key);
    }

    default byte[][] rawKeyArray(Collection<String> keys) {
        assertNotNull(keys, "cache keys not be empty");
    ...
    default List<byte[]> rawKeys(Collection<String> keys) {
        assertNotNull(keys, "cache keys not be empty");
        List<byte[]> keyBytes = new ArrayList<>(keys.size());
        for (String k : keys) {
            keyBytes.add(rawKey(k));
        }
        return keyBytes;
    }

    @SuppressWarnings("unchecked")
    default <T extends Serializable> T deserializeKey(byte[] keyBytes) {
        if (keyBytes != null) {
            return (T) getKeySerializer().deserialize(keyBytes);
        }
        return null;
    }

    @SuppressWarnings("unchecked")
    default <T extends Serializable> Set<T> deserializeKeys(Set<byte[]> bytes) {
        if (bytes == null) {
            return null;
        }
        Set<T> set = new HashSet<>(bytes.size());
        for (byte[] bs : bytes) {
            set.add((T) getKeySerializer().deserialize(bs));
        }
        return set;
    }
     ...
     * @return
     */
    default byte[] rawValue(Object value) {
        assertNotNull(value, "cache value not be null");
     ...
     * @return
     */
    default List<byte[]> rawValues(Collection<?> values) {
        assertNotNull(values, "cache values not be emtpy");
        List<byte[]> keyBytes = new ArrayList<>(values.size());
        for (Object k : values) {
            keyBytes.add(rawValue(k));
        }
     ...
     * @return
     */
    default <T> byte[][] rawValues(T[] array) {
        byte[][] bytes = new byte[array.length][];
        for (int i = 0; i < array.length; i++) {
            bytes[i] = rawValue(array[i]);
        }
        return bytes;
    }
     ...
     * @return byte[][]
     */
    default <T> byte[][] rawValueArray(Collection<?> set) {
     ...
     */
    default <V> Map<byte[],byte[]> rawMap(Map<String,V> map) {
        if (map == null) {
            return null;
        }
        Map<byte[],byte[]> byteMap = new HashMap<>(map.size() * 2);
        map.forEach((k, v) -> {
            byteMap.put(rawKey(k), rawValue(v));
        });
        return byteMap;
    }
     ...
     * @return
     */
    default <V> byte[][] rawKeyValueMap(Map<String,V> pair) {
        byte[][] kvBytes = new byte[pair.size() * 2][];
        int i = 0;
        for (Map.Entry<String,V> kv : pair.entrySet()) {
            kvBytes[i++] = rawKey(kv.getKey());
            kvBytes[i++] = rawValue(kv.getValue());
        }
        return kvBytes;
    }

    @SuppressWarnings("unchecked")
    default <T extends Serializable> T deserializeValue(byte[] valueBytes) {
        if (valueBytes != null) {
            return (T) getValueSerializer().deserialize(valueBytes);
        }
        return null;
    }

    @SuppressWarnings("unchecked")
    default <T> Set<T> deserializeValues(Set<byte[]> bytes) {
        if (bytes == null) {
            return null;
        }
        Set<T> set = new HashSet<>(bytes.size());
        for (byte[] bs : bytes) {
            set.add((T) getValueSerializer().deserialize(bs));
        }
        return set;
    }

    @SuppressWarnings("unchecked")
    default <T> List<T> deserializeValues(Collection<byte[]> valueBytes) {
        if (CollectionHelper.isNotEmpty(valueBytes)) {
            List<T> values = new ArrayList<>();
            for (byte[] bs : valueBytes) {
                values.add((T) getValueSerializer().deserialize(bs));
            }
            return values;
        }
        return null;
    ...
    public IRedisSerializer<Object> getValueSerializer();
     ...
     * @param map
     */
    void hputAll(final String key, final Map<String,? extends Serializable> map);
     ...
     * @return
     */
    <T> T runSHAScript(String luaScript, List<String> keys, List<? extends Serializable> args, Class<T> resultClazz);
     ...
     */
    <T> T runSHAScript(String luaScript, List<String> keys, List<? extends Serializable> args, Class<T> resultClazz,
            IRedisSerializer<T> resultSerializer);
